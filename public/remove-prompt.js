...
(function () {
  function isTerminalLang(lang) {
    ...
    return ["bash", "sh", "zsh", "shell", "shellsession", "console"].includes(
      l
    );
  }
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      const button =
        target && (target.closest ? target.closest("button[data-code]") : null);
      if (!button) return;
    ...
    navigator.clipboard.writeText = async function (text) {
      try {
        ...
        const terminalContext =
          isTerminalLang(lang) || isTerminalFigure(active);
...
(function () {
  function addDollarClass() {
    const codeBlocks = document.querySelectorAll(
      "pre[data-language] code > div.ec-line > div.code > span:first-child"
    );
